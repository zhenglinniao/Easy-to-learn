...
- **限流与配额**：游客每天有 3 次有效 AI 请求且每 5 分钟最多请求一次；登录用户每天有 10 次，不设请求间隔。
...
数据保留任务按每天一次执行，UTC `0 19 * * *` 对应北京时间凌晨 3 点，兼容当前 Vercel Hobby 计划并满足期满后 24 小时内清理的要求。调度请求仍通过 `CRON_SECRET` 鉴权。
