...
- 限流：Upstash Redis，使用原子脚本完成游客 5 分钟窗口、全身份每日配额和 requestId 幂等；登录用户不设请求间隔。
- AI：通过统一 `TutorModel` 接口接入 Gemini 与 OpenAI-compatible 服务；后者同时支持 Chat Completions 与 Responses API。DeepSeek `deepseek-flash` 使用 Responses API 的图文输入与 `json_schema`，并以 `reasoning.effort=none` 满足交互延迟。按 `AI_PROVIDERS` 顺序执行，只有超时、网络或供应商故障才自动回退。模型输出继续经过同一 Tutor DSL 校验，实际 provider/model 写入元数据。
...
- Prompt `v3` 把 Solve 扩展为通用教育拆解；`v4` 增加视觉优先约束；默认 Prompt `v5` 要求每一步同时包含有信息量的图解和简短解释，并新增安全 `code` 与 `part-map`，用于编程语法、物体部件、材料、食物叠层和爆炸拆解。数学内容继续把准确公式、几何或坐标关系与受控 Q 版 `comic-strip` 结合，前端统一绘制小易、步骤导航与动效，模型仍不得输出任意 HTML/SVG/CSS。习题继续遵循 `v2` 的答案位置策略，非习题不伪造单一答案。
- 手绘线条、逐步揭示、吉祥物动作与 reduced motion 属于前端表现层；模型只用 Tutor DSL 表达语义关系，不输出动画、HTML 或任意 SVG。
- Explain step 请求携带经过 Domain Schema 验证的父标题与目标步骤内容；仅传父 ID/步骤 ID 不足以保证模型上下文一致性，因此不再作为完整上下文。
...
type TutorBlock =
  ...
  | { type: "code"; language: "text" | "javascript" | "typescript" | "java" | "python" | "sql" | "c" | "cpp"; code: string }
  | { type: "list"; style: "ordered" | "unordered"; items: string[] }
  | { type: "callout"; tone: "info" | "warning" | "success"; text: string }
  | { type: "diagram"; diagram: DiagramV1 };

type DiagramV1 = CoordinatePlaneV1 | GeometryDiagramV1 | FlowDiagramV1 | ComicStripV1 | PartMapV1;
...
interface TutorResultV1 {
  ...
  hintLevel?: 3;
  contentProfile?: {
    contentKind: "exercise" | "question" | "article" | "food_dish" | "produce" | "object" | "process" | "diagram" | "mixed" | "unknown";
    learningGoal: "solve" | "explain" | "summarize" | "recipe" | "nutrition" | "production" | "growth" | "mechanism" | "compare" | "explore";
    goalSource: "explicit" | "inferred";
    confidence: "high" | "medium" | "low";
  };
  answerPresentation?: {
    problemType: "simple" | "reasoning";
    conclusionPosition: "first_step" | "final_step";
  };
  metadata: {
    model: string;
...
```json
{
  "data": {
    ...
    "quota": {
      ...
      "remaining": 3,
      "nextAllowedAt": null,
      "action": { "dailyLimit": 3, "dailyRemaining": 3, "periodLimit": 15, "periodRemaining": 15, "nextAllowedAt": null, "dailyResetsAt": "2026-09-26T16:00:00.000Z", "periodResetsAt": null },
      "image": { "dailyLimit": 1, "dailyRemaining": 1, "periodLimit": 3, "periodRemaining": 3, "periodResetsAt": null },
      "mode": "full"
    }
  }
}
...
### 8.4 `GET /api/ai/quota`

认证方式与 Tutor API 相同。返回每日、固定 30 天产品请求额度、独立插画额度、冷却结束时间与 `full/vector_only/paused` 模式。计数只在服务端 Redis 中读取，前端不得自行推算或写入。

### 8.5 `POST /api/ai/feedback`
...
| 429 | `RATE_LIMITED` | 是 | 游客 5 分钟窗口未结束 |
| 429 | `DAILY_QUOTA_EXHAUSTED` | 否 | 当日 3 次用尽 |
| 429 | `PERIOD_QUOTA_EXHAUSTED` | 否 | 固定 30 天产品请求额度用尽 |
| 429 | `QUOTA_EXCEEDED` | 否 | 旧客户端兼容错误码，新服务不再主动返回 |
