...
- Prompt `v3` 把 Solve 扩展为通用教育拆解；Prompt `v4` 在此基础上成为默认版本，按照认知依赖自动选择单步或多步，并要求每一步至少包含一个有信息量的图解。数学内容把准确公式、几何或坐标关系与受控 Q 版 `comic-strip` 结合，前端统一绘制小易、步骤导航与动效，模型仍不得输出任意 HTML/SVG/CSS。习题继续遵循 `v2` 的答案位置策略，非习题不伪造单一答案。
