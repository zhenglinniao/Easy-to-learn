...
- 浏览器只能持有 Supabase anon key；Gemini、Upstash、Sentry auth token 和 Supabase service role 只能在服务端。
...
- AI：主模型 `gemini-3.6-flash`，备用 `gemini-3.5-flash` 默认关闭。
...
依赖方向固定为：页面/feature 可以依赖 `domain`、`canvas-adapter`、`ui`；`canvas-adapter` 只依赖 Excalidraw 公共包和 `domain` 的必要值对象；`domain` 不依赖 React、DOM、Supabase 或 Gemini；API 依赖 `domain`，不依赖 Web。
...
入参：`requestId`、`rating: -1 | 1`、可选 `category` 枚举。无自由文本和附件。成功返回 `204`；重复 requestId 为幂等更新。
...
| 502 | `AI_PROVIDER_ERROR` | 是 | Gemini 非预期失败 |
...
- 日志 actor 使用带独立盐的不可逆摘要；request ID 贯穿浏览器、API、Redis 和 Gemini metadata。
...
| KaTeX | 0.18.7 | 数学公式 |
| `@google/genai` | 2.24.0 | Gemini server SDK |
...
服务端：`GEMINI_API_KEY`、`AI_MODEL`、`AI_FALLBACK_MODEL`、`AI_PROMPT_VERSION`、`ANON_SESSION_SIGNING_KEYS`、`UPSTASH_REDIS_REST_URL`、`UPSTASH_REDIS_REST_TOKEN`、`SUPABASE_URL`、仅上传/删除函数可用的 `SUPABASE_SERVICE_ROLE_KEY`、`SENTRY_AUTH_TOKEN`、`ALLOWED_ORIGINS`。
...
- API：MSW/fixture Gemini，覆盖认证、限流、幂等、超时、非法输出和日志脱敏。
...
- 依赖版本和编码前兼容性尖峰已定义。
- 本文确认后进入阶段 3；任何范围、协议、权限或数据保留变更必须新增 ADR。
