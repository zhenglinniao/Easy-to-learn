...
简单题默认一张主插画；复杂题最多生成三张插画。超过三步时，其余步骤使用现有 `comic-strip`、`part-map`、流程图和原创“小易”姿态，保证每步图文结合而不让一次请求无限增加成本与等待时间。
...
- 一次用户“解题/提示/解释步骤”仍只扣一次产品 AI 次数；内部规划和生图不分别扣次数。
- 简单题最多 1 张生成图，复杂题最多 3 张；默认 2K 以下展示尺寸，不为画布生成 4K 原图。
